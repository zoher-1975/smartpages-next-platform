    ...
    return (
      <a
        ...
        className={`inline-flex items-center justify-center gap-2
                    ...
                    shadow-[0_4px_12px_rgba(37,211,102,0.35)] ${className}`}
      >
        <WAIcon size={16} />
        Enquire
      </a>
...
// Generic store contact (no product)
export function WhatsAppContact({
  whatsapp, label = 'Chat on WhatsApp', className = '',
}: { whatsapp: string; label?: string; className?: string }) {
  const msg = encodeURIComponent('Hello! I found your store on Smart Pages and would like to know more. 👋')
  return (
    <a
      href={`https://wa.me/${whatsapp}?text=${msg}`}
      target="_blank"
      rel="noopener noreferrer"
      className={`inline-flex items-center gap-2 text-[#25D366] font-semibold
